...
class GraficasTest1 : AppCompatActivity() {
    ...
    private var datosPendientesDeGuardar = false
    private var accionMap: HashMap<String, Any>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnComprar.setOnClickListener {
            ...
            datosPendientesDeGuardar = true

            guardarAccionEnFirebase()
        }
    }
    ...
    private fun fetchStockDataFromApi(ticker: String, callback: (List<StockPrice>?, String?) -> Unit) {
        ...
        val url =
            "https://api.tiingo.com/tiingo/daily/$ticker/prices?startDate=2024-01-01&endDate=2024-06-01&token=$TOKEN_TINGO2"
    ...
    private fun guardarAccionEnFirebase() {
        accionMap?.let { map ->
            FirebaseFirestore.getInstance().collection("acciones")
                .add(map)
                .addOnSuccessListener {
                    datosPendientesDeGuardar = false
                    Toast.makeText(this, "Acción guardada en Firebase", Toast.LENGTH_SHORT).show()
